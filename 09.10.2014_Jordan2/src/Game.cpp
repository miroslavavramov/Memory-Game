...
//		start the main loop
	...
	SoundsBank::sound->initMusic("sounds/removeCards.wav");
	...
	m_StartGameMenu.backGroundForTeam.InitStatistic(m_pRenderer);
	m_StartGameMenu.backGroundForTeam.InitTeam(m_pRenderer);
	m_StartGameMenu.backGroundForTeam.InitRules(m_pRenderer);
//	m_StartGameMenu.backGroundMenu.InitStatistic(m_pRenderer);
...
//		m_temp.Init(m_pRenderer);
	...
	m_Credits.setDestination(332, 608, 291, 42);

	m_Up.Init(m_pRenderer);
	m_Up.setSource(0,110,35,38);
	m_Up.setDestination(152,685,35,38);

	m_Down.Init(m_pRenderer);
	m_Down.setSource(0,148,35,38);
	m_Down.setDestination(152,725,35,38);

	txt.Set(470, 90, 55, 50);
	txt.setTextColor(255, 255, 255, 255);

	txtGameOver.Set(260,230,500,300);
	txtGameOver.setTextColor(255, 0, 0, 255);

	level.Set(520, 30, 17, 35);
	level.setTextColor(254, 255, 88, 255);

//	**************
//	bet.Set();
//	bet.setTextColor(255, 255, 255, 255);
	credit.Set(420, 615, 35, 35);
	credit.setTextColor(255, 255, 255, 255);

	profit.Set(915, 700, 40, 40);
	profit.setTextColor(255, 255, 255, 255);

	bet.Set(55,713,40,40);
	bet.setTextColor(255,255,255,255);
...
//*********************
	...
	case GAMEOVER:
		...
		{
			txtGameOver.UpdateTextMessage("GAME OVER");
		}
		...
		if(m_stat.getBet() >= 5)
		{
			if(m_stat.getCredit() > m_stat.getBet())
			{
				if(m_Up.isIsClicked())
				{
					m_stat.setBet(5);
					m_Up.setIsClicked(false);
				}
			}

			if(m_stat.getBet() > 5)
			{
				if(m_Down.isIsClicked())
				{
					m_stat.setBet(-5);
					m_Down.setIsClicked(false);
				}
			}
		}
		...
		bet.IntToTextMessage(m_stat.getBet());
...
void Game::Draw() {
	...
	case STARTGAME:
		...
		m_Up.Draw(m_pRenderer);
		m_Down.Draw(m_pRenderer);
