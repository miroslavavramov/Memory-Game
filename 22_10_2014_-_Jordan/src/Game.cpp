...
//Create texture from surface pixels
	...
	PlayButton.Init(m_pRenderer);
	m_buttonsMenu.Init(m_pRenderer);
	m_buttonsMenuRecover.Init(m_pRenderer);
	...
	m_buttonsMenuRecover.setSource(0,104,232,52);
	m_buttonsMenuRecover.setDestination(100,250,260,50);


//	PlayButton = m_buttonSet.at(0);
	PlayButton.setSource(1160, 0, 232, 52);
...
void Game::Update() {
	...
	case MENU:
		...
		for (int iter = iterButt; iter < 4; iter++) {
...
void Game::Draw() {
	...
	case MENU:
		...
		{
			m_buttonSet[0].Draw(m_pRenderer);
		}
